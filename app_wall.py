...
def dfs(map_size: int, starting_node: Node, goal_pos: list, wall_pos: list):
    controller = Controller(map_size, wall_pos)
    node = starting_node
    node_count = 0

    while True:
        block_pos = [node.a_pos, node.b_pos, node.c_pos]
        potential_moves = controller.get_possible_moves(node.agent_pos, block_pos)
        ...
        new_pos = controller.move_agent(node.agent_pos, block_pos, move)
        node = Node(
...
def bfs(map_size: int, starting_node: Node, goal_pos: list, wall_pos: list):
    controller = Controller(map_size, wall_pos)
    node_count = 0
    depth = 0
    ...
    while queue:
        ...
        if [node.a_pos, node.b_pos, node.c_pos] == goal_pos:
            ...
            block_pos = [node.a_pos, node.b_pos, node.c_pos]
            possible_moves = controller.get_possible_moves(node.agent_pos, block_pos)
...
def ids(map_size: int, starting_node: Node, goal_pos: list, wall_pos: list):
    depth_limit = 0
    node_count = 0
    controller = Controller(map_size, wall_pos)
    stack = [starting_node]
...
def a_star(map_size: int, starting_node: Node, goal_pos: list, wall_pos: list):
    # Heuristic function
...
output = a_star(4, test_node, goal, [(2, 1), (2, 2)])
